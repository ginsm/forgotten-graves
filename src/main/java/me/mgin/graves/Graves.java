...
public class Graves implements ModInitializer {
	...
	public static final GraveBase GRAVE_FORGOTTEN = new GraveBase(BlockAge.FORGOTTEN, FabricBlockSettings.of(Material.ORGANIC_PRODUCT).strength(0.8f, -1f));
	public static final GraveBase GRAVE_WEATHERED = new GraveBase(BlockAge.WEATHERED, FabricBlockSettings.of(Material.ORGANIC_PRODUCT).strength(0.8f, -1f));
	public static final GraveBase GRAVE_OLD = new GraveBase(BlockAge.OLD, FabricBlockSettings.of(Material.ORGANIC_PRODUCT).strength(0.8f, -1f));
	public static final GraveBase GRAVE = new GraveBase(BlockAge.FRESH, FabricBlockSettings.of(Material.ORGANIC_PRODUCT).strength(0.8f, -1f));
	...
	public static String BRAND_BLOCK = "grave";//Maybe name can be changed in config? Also helps for diy quick and easy locale. Plus people who want to call it a gravestone can.
	//tbh I'm tired
	...
	public void onInitialize() {
		...
		GRAVE_BLOCK_ENTITY = Registry.register(Registry.BLOCK_ENTITY_TYPE, MOD_ID+":"+BRAND_BLOCK, FabricBlockEntityTypeBuilder.create(GraveBlockEntity::new, GRAVE,GRAVE_OLD,GRAVE_WEATHERED,GRAVE_FORGOTTEN).build(null));
		Registry.register(Registry.ITEM, new Identifier(MOD_ID, BRAND_BLOCK), new BlockItem(GRAVE, new Item.Settings().group(ItemGroup.DECORATIONS)));
		Registry.register(Registry.ITEM, new Identifier(MOD_ID, BRAND_BLOCK+"_old"), new BlockItem(GRAVE_OLD, new Item.Settings().group(ItemGroup.DECORATIONS)));
		Registry.register(Registry.ITEM, new Identifier(MOD_ID, BRAND_BLOCK+"_weathered"), new BlockItem(GRAVE_WEATHERED, new Item.Settings().group(ItemGroup.DECORATIONS)));
		Registry.register(Registry.ITEM, new Identifier(MOD_ID, BRAND_BLOCK+"_forgotten"), new BlockItem(GRAVE_FORGOTTEN, new Item.Settings().group(ItemGroup.DECORATIONS)));
	...
	public static void placeGrave(World world, Vec3d pos, PlayerEntity player) {
		...
		for (BlockPos gravePos : BlockPos.iterateOutwards(blockPos.add(new Vec3i(0, 1, 0)), 5, 5, 5)) {
			...
			if(canPlaceGrave(world, block, gravePos)) {
				...
				int currentExperience = ExperienceCalculator.calculateTotalExperience(
				...
				System.out.println("[Graves] Grave spawned at: " + gravePos.getX() + ", " + gravePos.getY() + ", " + gravePos.getZ());
				break;
			}
		}
	}
