...
public class RetrieveGraveTest {
     ...
     * - NBT items not consolidating with their non-nbt counterparts.<br>
     * - Non-stackables not consolidating (i.e. enchanting books).
     */
    ...
    public static void overflowRetrieval(TestContext context, PlayerEntity player, BlockPos pos) {
        ...
        String currentInventory = "[{Slot:0b,id:\"minecraft:grass_block\",Count:64b},{Slot:1b,id:\"minecraft:grass_block\",Count:64b},{Slot:2b,id:\"minecraft:grass_block\",Count:64b}]";

        System.out.println(">> Running overflowRetrieval <<");

        // Overflows because grave inventory is full (merge order GRAVE)
        GraveTestHelper.runCommand(context, "graves server config set mergeOrder GRAVE");
        checkOverflow(context, player, pos, currentInventory, fullInventory, Items.GRASS_BLOCK, 192);
